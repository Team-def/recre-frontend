...
export default function CatchPlayer({
  ...
  useEffect(() => {
    ...
    socket.on("correct", (res) => {
      ...
      setModalContent(
        <div>
          {res.nickname}님께서 정답을 맞추셨습니다!<br></br> 정답 : {res.answer}
        </div>
      );
      setOpen(true);
      setIsCorrect(true);
  ...
  return (
      ...
      <div className="p_catch_div">
        <div className="submitDiv">
          <TextField
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start" sx={{ fontFamily: "myfont" }}>
                  정답 :{" "}
                </InputAdornment>
              ),
            }}
          ...
          <Button
            className="throw-answer"
            variant="contained"
            onClick={throwCatchAnswer}
            disabled={buttonDisabled}
            sx={{ fontFamily: "myfont" }}
          >
            {" "}
            {buttonDisabled ? "대기" : "제출"}
          </Button>
        </div>
        <MyModal
          open={open}
    ...
    >
        ...
        <Button
          ...
          variant="contained"
          onClick={leave_game}
          sx={{ fontFamily: "myfont", position: "static", marginBottom: "5%" }}
        >
          게임 나가기
        </Button>
      </div>
